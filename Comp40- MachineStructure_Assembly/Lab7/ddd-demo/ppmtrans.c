...
void rotate180 (int i, int j, A2 a, void* elem, void* newI) {
  ...
  A2 temp = newI;
  Pnm_rgb dest =
    methods->at(temp, (methods->width(a) - i - 1), (methods->height(a) - j - 1));
  *dest = *src;
}
...
int main(int argc, char *argv[]) {
  ...
  void (*map)(A2 pixmap, void apply(int, int, A2, void*, void*), void *cl)
    = methods->map_default;
...
#define SET_METHODS(METHODS, MAP, WHAT) do { \
  ...
  newImage->pixels = methods->new(newImage->width, newImage->height, sizeof(*temp));
