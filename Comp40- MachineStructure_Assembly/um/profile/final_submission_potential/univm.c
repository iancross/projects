...
extern UnivM_T UnivM_new(FILE* program){
        ...
        segMem_map(machine->segments, machine->freed_mem, machine->regs,
                        machine->tail, machine->num_segments, SIZE_INDEX, SEG0_INDEX);
...
extern void UnivM_free(UnivM_T machine){
        ...
        uint32_t seq_length = *((uint32_t*)machine->tail);
...
{       
        ...
        if (opcode == 0){
                ...
                exit(0);
                        ...
                        machine->tail, machine->num_segments, rc, rb);}
...
static uint32_t get_inst(FILE* program){
        ...
        for(int i = 0; i < 4; i++){
                ...
                curr_bits >>= (WORD_SIZE - width);
                right_shifted_word = curr_word >> (width + lsb) << (width + lsb);
                left_shifted_word = curr_word << (64 - lsb) >> (64 - lsb);
                curr_bits <<= lsb;
                if(lsb == 0){ curr_word = (right_shifted_word | curr_bits);}
                else {curr_word = (right_shifted_word | left_shifted_word) | curr_bits;}
                lsb -= 8;
        }
        return curr_word;
}
