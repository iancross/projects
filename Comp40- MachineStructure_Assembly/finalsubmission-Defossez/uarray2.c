...
void UArray2_map_row_major(T input, void apply(int x, int y, void *val,
                           void *cl), void *cl)
{
        assert(input);
        ...
        for(i = 0; i < numElems; i++) {
                apply(x_from_i(i, input), y_from_i(i, input), 
                      UArray_at(input->array, i), cl);
        }
}

void UArray2_map_col_major(T input, void apply(int x, int y, void *val,
                           void *cl), void *cl)
{
        assert(input);
        int x, y;
        for(x = 0; x < input->width; x++) {
                for(y = 0; y < input->height; y++) {
                        apply(x, y, UArray2_at(input, x, y), cl); 
                }
        }
}
