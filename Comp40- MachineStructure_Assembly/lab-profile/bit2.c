...
struct T {
  int width, height;
  UArray_T rows; // Array_T of 'height' Bit_Ts, each of length 'width'
   // Element (i, j) in the world of ideas maps to
   //   bit number 'i' in *(Bit_T*)Array_get(rows, j)
};
...
T Bit2_new(int width, int height) {
  ...
  for (i = 0; i < height; i++) {
    Bit_T b = Bit_new(width);
    Bit_T *p = UArray_at(bitmap->rows, i);
    *p = b;
  }
  assert(is_ok(bitmap));
  return bitmap;
}
#line 84 "www/solutions/bit2.nw"
...
#line 112 "www/solutions/bit2.nw"
...
#line 141 "www/solutions/bit2.nw"
...
#line 153 "www/solutions/bit2.nw"
